...
function queueWatcher (watcher) {                       //watcher队列只有一个目的，防止重复的watcher被执行，  当然了，当这个队列全部执行完成之后，会自动清空，   再执行上次出现的watcher是可以的。  （那么就是防止短时间内重复执行watcher）
  ...
  if (has$1[id] == null) {
    ...
    if (!flushing) {                                                     // 放入队列而已， 如果还没有刷新，直接放进去， 如果已经开始刷新了  需要排序进去。
      ...
      queue.splice(Math.max(i, index$1) + 1, 0, watcher);                      // watcher 是按照id进行排序的， 所以按照id插入到响应的位置。
    }
    // queue the flush
    if (!waiting) {
      waiting = true;
...
var Watcher = function Watcher (
  ...
  this.cb = cb;                                                    //回调
  this.id = ++uid$1; // uid for batching                             //每一个watcher 都有一个id
...
function initState (vm$$1) {
  ...
  if (opts.data) {
    initData(vm$$1);                                        // 大家请看， 都别走，都别走， 从这里开始了数据响应系统
  } else {
...
function initData (vm$$1) {
  ...
  data = vm$$1._data = typeof data === 'function'                                       // 在实例对象上添加vm._data 属性，  和vm.$options.data 是相同的引用。
    ? data.call(vm$$1)
    : data || {};
  ...
  while (i--) {                                                                         //   循环的目的是在实例对象上对数据进行代理，这样我们就能通过 this.a 来访问 data.a 了
    if (props && hasOwn(props, keys[i])) {
      ...
      proxy(vm$$1, keys[i]);                                                               // 在实例对象上设置与 data属性同名的访问器属性，然后使用      _data 做数据劫持
    }
  }
  // observe data
...
function stateMixin (Vue) {
  ...
  Vue.prototype.$watch = function (                                   //$watch 的都是实例属性， 都可以在创建vue后  用vm.$watch调用this.$watch(function(){}, (newVal, oldVal) => {}）
    ...
    options = options || {};
    options.user = true;
    var watcher = new Watcher(vm$$1, expOrFn, cb, options);           // 外部的调用Watcher
...
function lifecycleMixin (Vue) {
  Vue.prototype._mount = function (
    ...
    callHook(vm$$1, 'beforeMount');                            // 触发 beforeMount 生命周期钩子
    vm$$1._watcher = new Watcher(vm$$1, function () {                  // 内部的Watcher收集变量，    vm._render()渲染成虚拟DOM     vm._update将虚拟DOM中的最后一步：patch到DOcument中
      vm$$1._update(vm$$1._render(), hydrating);                  // update()中没有触发 data.get  ,  那么必须render()中触发了get，  否则不会被Dept收集
    }, noop);                                               // new Watcher() 是新检测一个属性，然后执行回调，  然而这里的回调是noop 显然这里不是为了响应的。  vm._update已经完成了所有了呀？  这里用Watcher有什么用呢？
    hydrating = false;
    // manually mounted instance, call mounted on self
    // mounted is called for render-created child components in its inserted hook
    if (vm$$1.$vnode == null) {                              // 如果是第一次mount则触发 mounted 生命周期钩子
...
function initRender (vm$$1) {
  ...
  if (vm$$1.$options.el) {                                                            // 只有在el存在的情况下，才去mount，  主要方法是vm.$mount(vm.$options.el)，如果没有el那么就不去mount，   初始化的时候可以不调用， 可以在以后中手动调用vm.$mount()
...
function renderMixin (Vue) {
  ...
  Vue.prototype._render = function () {
    ...
    var vnode;
    try {
      vnode = render.call(vm$$1._renderProxy, vm$$1.$createElement);                     // render执行之后，创建虚拟DOM对象   Vue提供了 render 选项，作为 template 的代替方案，
    } catch (e) {
      /* istanbul ignore else */
      if (config.errorHandler) {
...
function initMixin (Vue) {
  Vue.prototype._init = function (options) {          // vue初始化  执行的第一个函数
    var vm$$1 = this;                                //这里的this  应该是指的是Vue的实例对象，  那么  vm = Vue 的一个实例
    // a uid
    vm$$1._uid = uid++;
    ...
    if (options && options._isComponent) {                     // 判断有没有已经使用过了_isComponent这个属性，因为内部要使用 ，怕被占用了
      ...
      vm$$1.$options = mergeOptions(                              //第一步要做的是 使用策略对象合并参数选项。 el 选项会使用 defaultStrat 默认策略函数处理，，data 选项则会使用 strats.data 策略函数处理
        resolveConstructorOptions(vm$$1.constructor),           // Vue.options   （如果定义了super可能会 对options进行修改）
        options || {},                                        //  options        是我们调用Vue传入的options
        vm$$1                                                    // Vue.prototype == this
      );
    }
    /* istanbul ignore else */
    if (process.env.NODE_ENV !== 'production') {
