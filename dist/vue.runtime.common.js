...
function Vue$2 (options) {                             //这就是定义的Vue 类   也就是构造函数                                所有这个文件夹的js  其实最后就生产一个Vue 类，   类上有很多函数。 var vm = new Vue({}）之后 执行了init函数。
  if (process.env.NODE_ENV !== 'production' &&
...
initMixin(Vue$2);             //下面这5个方法 分别在Vue.prototype上面添加属性和方法。
stateMixin(Vue$2);
eventsMixin(Vue$2);
...
initGlobalAPI(Vue$2);                                                //在Vue构造函数上添加静态属性和方法。

Object.defineProperty(Vue$2.prototype, '$isServer', {            // Vue.prototype 上挂载了 $isServer
  get: isServerRendering
});

Vue$2.version = '2.4.4';                                 //在 Vue 上挂载了 version 属性。
...
Vue$2.prototype.__patch__ = inBrowser ? patch$1 : noop;        // 打补丁，补漏洞， 这是vue发生改变， 需要更新的时候， 去打补丁
