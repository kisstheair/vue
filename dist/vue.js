 ...
 */
var isBuiltInTag = makeMap('slot,component', true);        // 返回的是一个函数isBuiltInTag，  这个函数 传入 solt  会返回true， 传入没有的返回 undefined
 ...
 */
var hasOwnProperty = Object.prototype.hasOwnProperty;                // 检测 是自己的属性，还是原型上的属性。
 ...
 */
function cached (fn) {                 // 传入的函数， 函数处理一下返回。
...
function looseIndexOf (arr, val) {       // 判断 一个值在数组中的位置。
  for (var i = 0; i < arr.length; i++) {
...
function defineReactive$$1 (
  ...
  Object.defineProperty(obj, key, {
    ...
    get: function reactiveGetter () {                                    // 先建立数据响应系统，date的每一个属性对应自己的一个Dept对象， data属性的属性也对应一个Dept对象， 数组的每一个元素对应一个Dept ，深遍历。
      var value = getter ? getter.call(obj) : val;                      // 建立响应系统之后，都可以用了，但是没有Dep.target，响应不会生效。   只有vm._watcher = new Watcher(这个执行，才会修改Dep.target， 这样就可以执行dep.depend() 了
      if (Dep.target) {
...
/*  */       //   如何合并一个父选项值和一个子选项  的 一些属性和函数。

var strats = config.optionMergeStrategies;                    // strats策略   == 选项融合策略。是一个对象{key，function}用来配置 怎么融合的。
...
config._lifecycleHooks.forEach(function (hook) {        // 把钩子函数数组 挂载到 策略上
 ...
 */
function mergeOptions (                 //mergeOptions根据融合策略把各个属性都融合到一起。  strats是融合策略的数组，data怎么融合  指令怎么融合，钩子怎么融合，都保存在strats数组中
  parent,                              //构造函数的option参数
  child,                              //实例传入的option参数
  vm
) {
  {
...
var has = {};
var circular = {};
var waiting = false;                                 //等待时间，  从开始执行到全部刷新结束，
var flushing = false;                                //正在刷新状态-----------遍历watcher执行。
var index = 0;
...
function flushSchedulerQueue () {
  ...
  queue.sort(function (a, b) { return a.id - b.id; });                                        //按照id进行排序的
 ...
 */
function queueWatcher (watcher) {                       //watcher队列只有一个目的，防止重复的watcher被执行，  当然了，当这个队列全部执行完成之后，会自动清空，   再执行上次出现的watcher是可以的。  （那么就是防止短时间内重复执行watcher）
  var id = watcher.id;
...
var Watcher = function Watcher (
  ...
  this.cb = cb;                                                    //回调
  this.id = ++uid$2; // uid for batching                             //每一个watcher 都有一个id
  this.active = true;                                             // 是否激活 也就是是否运行callback的标识
  this.dirty = this.lazy; // for lazy watchers                       是不是暂时先不触发回调的
  this.deps = [];                                                  //都有那个收集器收集了这个watcher
  this.newDeps = [];                                               //新添加的
  this.depIds = new _Set();                                         //做缓存用的吗？
  ...
  if (typeof expOrFn === 'function') {
    ...
    this.getter = parsePath(expOrFn);                            //expOrFn可能是a.b.c  返回的是一个可以深层次查找属性的函数     如：expOrFn：athis.getter = (obj)=》{retuen obj.a}
    if (!this.getter) {
 ...
 */
Watcher.prototype.get = function get () {                                                   // 带有防止重复的处理，添加了一次依赖之后，后面的不会再添加。
  pushTarget(this);
  var value = this.getter.call(this.vm, this.vm);     // expOrFn可能是a.b  传入的是this.vm对象  那么函数执行会查找 this.vm.a   this.vm.a.b ------》触发get属性
...
/**
 * Add a dependency to this directive.                把dep放入本watcher中，  data 的每一个属性都对应自己的Dep对象，watcher在查找那一个属性的时候可能触发一串的get 所以对应多个Dep
 */
...
Watcher.prototype.run = function run () {
  if (this.active) {
    var value = this.get();                      //为什么要再次调用get()函数呢？
    if (
      value !== this.value ||
...
function stateMixin (Vue) {
  ...
  Vue.prototype.$watch = function (                                   //$watch 的都是实例属性， 都可以在创建vue后  用vm.$watch调用this.$watch(function(){}, (newVal, oldVal) => {}）
    ...
    options = options || {};
    options.user = true;
    var watcher = new Watcher(vm, expOrFn, cb, options);           // 外部的调用Watcher
...
function lifecycleMixin (Vue) {
  Vue.prototype._mount = function (
    ...
    vm._watcher = new Watcher(vm, function () {                  // 内部的Watcher收集变量，  newWatcher 中触发了get被Dept收集                      相应于一个vm实例只对应一个watcher，
  ...
  Vue.prototype._update = function (vnode, hydrating) {
    ...
    var prevEl = vm.$el;                      //之前的HTML元素
    var prevVnode = vm._vnode;                 //之前的vnode  虚拟dom
...
function renderMixin (Vue) {
  ...
  Vue.prototype._render = function () {
    ...
    var _parentVnode = ref._parentVnode;                                           // 解构出 中的 render 函数  $options  是合并属性之后， 编译模板之后的所有属性加载的地方。
...
function initMixin (Vue) {
  Vue.prototype._init = function (options) {          // 这里只是把 _init函数加到Vue原型对象上面，并没有执行，  vue实例化之后， 执行的第一个函数
    var vm = this;                                //这里的this  _init函数执行是在 构造函数中执行的， this._init(options)  所以这里额this 指的是实例对象。
    // a uid
    vm._uid = uid++;
...
function resolveConstructorOptions (Ctor) {      //传入的是Vue构造函数
  ...
  if (Ctor.super) {                                            //vue是可以继承的， 如果是继承过来的， 继承的方式是Vue.extend = function (extendOptions: Object) ，所以多添加了一些option选项，并且构造函数上也有了super属性，指向父
    var superOptions = Ctor.super.options;                   //把继承之后的参数，和父的参数合并一下。
...
function Vue$3 (options) {                             //这就是定义的Vue 类   也就是构造函数                                所有这个文件夹的js  其实最后就生产一个Vue 类，   类上有很多函数。 var vm = new Vue({}）之后 执行了init函数。
  if ("development" !== 'production' &&
    !(this instanceof Vue$3)) {
    warn('Vue is a constructor and should be called with the `new` keyword');
  }
  this._init(options);       // 这里并没有执行，而是生成对象的时候才去执行的。
}

initMixin(Vue$3);             //下面这5个方法 分别在Vue.prototype上面添加属性和方法。
stateMixin(Vue$3);
eventsMixin(Vue$3);
...
function initExtend (Vue) {
  ...
  Vue.extend = function (extendOptions) {
    ...
    var Sub = function VueComponent (options) {                    //创建一个空函数， 照搬Vue上面的属性，   主要就是合并继承的option   extendOptions，  初始化的时候调用自己的init；
      this._init(options);
    };
...
function initGlobalAPI (Vue) {
  ...
  extend(Vue.options.components, builtInComponents);     //内建立组件，KeepAlive，  每一个vue实例都会有的
...
initGlobalAPI(Vue$3);                                                //在Vue构造函数上添加静态属性和方法。

Object.defineProperty(Vue$3.prototype, '$isServer', {            // Vue.prototype 上挂载了 $isServer
  get: isServerRendering
});

Vue$3.version = '2.4.4';                                 //在 Vue 上挂载了 version 属性。

                                           //到这一步才是真的导出Vue，前面的都是生产文件代码。
...
var mustUseProp = function (tag, attr) {         //必须更新的属性
  return (
...
function createElement$1 (tagName, vnode) {                   // vnode 转化为 html 的element 的方法   创建一个普通的element
...
function createElementNS (namespace, tagName) {            // 创建带有命名空间的  element 只针对  xml
  return document.createElementNS(namespaceMap[namespace], tagName)
}

function createTextNode (text) {                                                //创建一个 text 节点
...
function insertBefore (parentNode, newNode, referenceNode) {                     //插入到某个元素的前面
...
function appendChild (node, child) {                                                    //添加子节点
  node.appendChild(child);
}

function parentNode (node) {                                                            //找到父节点
  return node.parentNode
}

function nextSibling (node) {                                                           //找到弟弟节点
  return node.nextSibling
}

function tagName (node) {                                                         // 返回tag名称
  return node.tagName
}

function setTextContent (node, text) {                                              //设置文本内容
  node.textContent = text;
}

function setAttribute (node, key, val) {                         //  设置element的属性
...
var ref = {                                 // 把 template 中的所有vnode 的 ref 属性  -----放到vm.$refs中    例如：ref = table   vm.$refs= [table:vnode.elm]      ref的值 对应的是这个vnode的 Element
...
function createPatchFunction (backend) {
  var i, j;
  var cbs = {};                                     // 存放的是各个 属性的数组  例如 ：{create:[fn1,fn2,fn3], update:[fn1,fn2,,,]}
  ...
  return function patch (oldVnode, vnode, hydrating, removeOnly, parentElm, refElm) {
    ...
    if (!oldVnode) {
      ...
      var isRealElement = isDef(oldVnode.nodeType);          //通过查看nodeType属性查看是不是真的HTML Element
...
var directives = {                                       // 这里是指令执行的地方，  当一个Vnode 创建，更新，销毁的时候，  会看看自己有没有指令，  如果有指令到这里来操作一把。
...
var platformModules = [                      //  在 vnode---Element 转变的时候，  vnode的值 ---怎么到 Element上面呢？   肯定不能直接用 “.”吧 ，   所以转换完之后要调用这些方法  把 vnode的值用相应的api 配置到Element上面。
  attrs,
  klass,
  events,
  domProps,
...
var modules = platformModules.concat(baseModules);                    // [attrs:{create:fn,uptade:fn,,,}]
...
var platformDirectives = {            //平台组件，  在 entries/web-runtime.js 中 提前给  Vue.options.directives  添加上了指令，     这说明VUe 在生成构造函数中   合并了这些指令，   每一个vm都有的。
  model: model,                       // runtime的指令是  配置生效时候的操作，
  show: show
};
...
var platformComponents = {                      //平台组件，  在 entries/web-runtime.js 中 提前给  Vue.options.components  添加上了组件，     这说明VUe 在生成构造函数中   合并了这些组件，   每一个vm都有的。
...
extend(Vue$3.options.directives, platformDirectives);    // 默认每一个实例都有的指令             // model,show  （runtime时候）
extend(Vue$3.options.components, platformComponents);     // 默认每一个vue实例都预先添加的组件   // Transition,TransitionGroup   （runtime时候）

// install platform patch function
Vue$3.prototype.__patch__ = inBrowser ? patch : noop;        // 打补丁，补漏洞， 这是vue发生改变， 需要更新的时候， 去打补丁
...
var isUnaryTag = makeMap(                              //是不是一元标签，自关闭 没有结束符号
...
var attribute = new RegExp(                          // 组合成属性的正则，可以匹配的结果  AA = 12，  BB = '123', CC = "345"
...
var qnameCapture = '((?:' + ncname + '\\:)?' + ncname + ')';        // 捕获名称
var startTagOpen = new RegExp('^<' + qnameCapture);                //开始标签打开的开始 <....
var startTagClose = /^\s*(\/?)>/;                                  // 开始标签的结束  空格>  或者 空格/>
var endTag = new RegExp('^<\\/' + qnameCapture + '[^>]*>');        // 结束标签 </fwefwer234>
var doctype = /^<!DOCTYPE [^>]+>/i;
var comment = /^<!--/;
var conditionalComment = /^<!\[/;                                   //条件注释  <![CDATA[]]>]]>这是一种， 不需要解析
...
function parseHTML (html, options) {
  ...
  while (html) {
    ...
    if (!lastTag || !isSpecialTag(lastTag, options.sfc, stack)) {
      var textEnd = html.indexOf('<');                             // 是不是以< 开头的，是就是tag， 不是就是文本
      if (textEnd === 0) {                                        // 此时字符串是以<开头        看一看是不是 html标签，如果是的话，截取一下然后 继续循环
        ...
        // Doctype:
        var doctypeMatch = html.match(doctype);                //  清除掉   <!DOCTYPE >
        if (doctypeMatch) {
        ...
        // Start tag:
        var startTagMatch = parseStartTag();        //匹配开始标签，有点复杂，开始标签 可能有很多属性
        if (startTagMatch) {
      ...
      if (textEnd > 0) {                                           // 此时字符串  不是以<开头  ，那就是文本
        ...
        while (
          !endTag.test(rest$1) &&                                 //不是 结束标签，不是开始标签，不是注释标签，不是条件注释标签，的情况下，   把这个<>  删除掉
          !startTagOpen.test(rest$1) &&
          !comment.test(rest$1) &&
          !conditionalComment.test(rest$1)
        ) {
          // < in plain text, be forgiving and treat it as text      在纯文本中，要宽恕并把它当作文本对待。   明明以< 开头了 但是呢去不是 tag，所以宽恕他为文本吧
          next = rest$1.indexOf('<', 1);                               //rest是剪切过文本的， 下面的应该是<开始的，   这里 从 1的位置开始算起，那就匹配下一个<
          if (next < 0) { break }                                     // 剩下的都是纯文本了， 不考虑 tag了
          textEnd += next;
          rest$1 = html.slice(textEnd);                                // 减掉一个tag  <>。那是什么标签呢？           //不是 结束标签，不是开始标签，不是注释标签，不是条件注释标签，的情况下，   把这个<>  删除掉
        }
  ...
  function parseStartTag () {
    var start = html.match(startTagOpen);            //匹配到开始标签的开始 <
    if (start) {
      var match = {                               // 匹配到 tag名字，属性，开始位置，结束位置
        tagName: start[1],
        attrs: [],
        start: index
      };
...
var dirRE = /^v-|^@|^:/;
var forAliasRE = /(.*?)\s+(?:in|of)\s+(.*)/;                           // 匹配for 循环的内容  item in sexlist
var forIteratorRE = /\((\{[^}]*\}|[^,]*),([^,]*)(?:,([^,]*))?\)/;     // for 迭代器 (item ,index)
var bindRE = /^:|^v-bind:/;
var onRE = /^@|^v-on:/;
var argRE = /:(.*)$/;
var modifierRE = /\.[^.]+/g;

var decodeHTMLCached = cached(decode);          // 解码（HTML），     里面可以缓存，
...
function processFor (el) {
  ...
  if ((exp = getAndRemoveAttr(el, 'v-for'))) {
    ...
    var iteratorMatch = alias.match(forIteratorRE);    // for的迭代器 看看哟没有 （item ，index）
    if (iteratorMatch) {
...
var baseDirectives = {                        //这里的指令是基础指令，　所有的节点都可由的指令
  bind: bind$1,
  cloak: noop
};
...
var staticRenderFns;                          // 静态执行函数，不需要数据变动之后重新渲染的
var onceCount;
var currentOptions;

function generate (　　　　　　　　　　　　　　　　　　　　　// 解释器， 把AST 解析成  html
  ast,
  options
) {
  ...
  platformDirectives$1 = options.directives || {};                            // 开始的时候平台指令已经放到 Vue.options.directives  ，实例化的时候可能也会添加自定义的指令，  这里是二者的合并。
...
function compile$1 (
  ...
  var ast = parse(template.trim(), options);            // 解析成抽象语法树      把html 模板 解析成 AST
  optimize(ast, options);                                  // 使最优化，使尽可能有效
  var code = generate(ast, options);                     // 构建成 code = {render函数,staticRenderFns}   render函数 是 with(this){return_c{.....}}    在哪个环境中执行，this就执行谁。
  return {
    ast: ast,
...
var directives$1 = {　　　　　　　　　　　// 这里的指令是平台指令，只有web平台才会有的
  model: model$1,                               // compiler时候的指令是，配置解析到template  有v-model 了  该怎么生成AST
  text: text,
  html: html
};
...
var baseOptions = {               //编译模板的基础参数
  expectHTML: true,
  modules: modules$1,
  staticKeys: genStaticKeys(modules$1),
  directives: directives$1,                               // 解析的时候， 平台可以有哪些指令？ v-html  v-model   v-text
  isReservedTag: isReservedTag,                           //  是不是保留标签     web的保留标签是 html,body,base,head,link,meta,style,title，，，所有html标签，  weex的标签 只有div,img,image,input,switch,indicator,list,scroller,cell,template,text,slider,image这么多
  isUnaryTag: isUnaryTag,                              //  是不是一元标签，自关闭 没有结束符号  web 有area,base,br,col,embed,frame,hr,img                    weex 没有
  mustUseProp: mustUseProp,                             // 那些标签必须拥有属性， selected 必须拥有option   input 必须拥有value
  getTagNamespace: getTagNamespace,                         // 获取标签的命名空间，  web 只有svg  的命名空间是svg ，                                              weex没有
  isPreTag: isPreTag                                 // 是不是 <pre></pre>   这个标签
};
...
function compile (
  ...
  options = options
    ...
    : baseOptions;
  return compile$1(template, options)            // 传入 模板 ---返回 {AST ，render函数，}
}
...
function compileToFunctions (
  ...
  var res = {};
  var compiled = compile(template, options);      //编译结果
...
var mount = Vue$3.prototype.$mount;               // 缓存了来自 web-runtime.js 的 $mount 方法
Vue$3.prototype.$mount = function (                // 重写 $mount 方法
  el,
  hydrating
) {
  ...
  if (!options.render) {                                     // 如果我们没有写 render 选项，那么就尝试将 template 或者 el 转化为 render 函数
    var template = options.template;                          //  这里最主要的目的就是生成render函数，如果没有render 那就查看有没有template----根据compileToFunctions  把模板转化为render
